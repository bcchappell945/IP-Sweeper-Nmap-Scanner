...
import os
import nmap
#This is a personal project I wanted to put some of my Python skills to the test, expirement with networking libraries, 
#and gain a better understanding of nmap. The concept for this tool is simple yet effective: look for all of the IPs on 
#a subnet, scan the ones that are online, and then store each IP scan into a file the user can come back to later. 
#It was a fun little project that does its job!
#This tool was made for ethical and legal uses only. Any other such use is not approved, unethical, and possibly illegal. 
#The tool is required to run as root or by using sudo. 
#This is because the nmap scan runs and grabs operating system information of the live hosts on the network, 
#which requires `sudo` or root to run. An example case of running this tool is `sudo python3 SweepScan.py 192.168.10.10`. 
      ...
      for ip in live_hosts:
            try:
                ...
                print("[-] Scan complete!") # Tell the user a scan is complete
                 ...
                 print("[!] Error: An error occurred scanning the hosts! Try running with sudo or as root!")
                 exit(6)
            try:
